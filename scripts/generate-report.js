...
function parseJUnitXml(xmlContent) {  
    ...
    for (let suite of testsuites) {  
        ...
        for (let test of testcases) {  
            ...
            // Clasificación mejorada de pruebas  
            const classNameLower = className.toLowerCase();  
            const testNameLower = testName.toLowerCase();  

            if (classNameLower.includes('security') || testNameLower.includes('security')) {  
                results.groupedTests.security.push(testData);  
            }  
            else if (classNameLower.includes('integration') || testNameLower.includes('integration')) {  
                results.groupedTests.integration.push(testData);  
            }  
            else if (classNameLower.includes('system') || testNameLower.includes('system')) {  
...
<body class="bg-gray-100 p-8">  
    <div class="max-w-7xl mx-auto">  
        ...
        ${generateTestSection(results.groupedTests.unit, 'Pruebas Unitarias')}  
        ${generateTestSection(results.groupedTests.integration, 'Pruebas de Integración')}  
        ${generateTestSection(results.groupedTests.security, 'Pruebas de Seguridad')}  
        ${generateTestSection(results.groupedTests.system, 'Pruebas de Sistema')}  
    </div>  
</body>  
</html>`;  

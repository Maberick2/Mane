...
export function isECR(registry: string): boolean {
  return (
    /\.dkr\.ecr\.[a-z0-9-]+\.amazonaws\.com(\.cn)?$/i.test(registry) ||
    isPubECR(registry)
  );
}
...
export function getRegion(registry: string): string {
  ...
  const match = registry.match(
    /\.dkr\.ecr\.([a-z0-9-]+)\.amazonaws\.com(\.cn)?$/i
  );
...
export async function getRegistriesData(
  registry: string
  ...
  if (isPubECR(registry)) {
    ...
    const auth = Buffer.from(
      response.authorizationData?.authorizationToken || '',
      'base64'
    )
      .toString()
      .split(':');
...
export async function login(
  ...
  const cleanValidation = validateCleanCredentials(
    cleanUsername,
    cleanPassword
  );
  if (cleanValidation) return cleanValidation;

  const lengthValidation = validateCredentialsLength(
    cleanUsername,
    cleanPassword
  );

...
export async function logout(registry: string): Promise<void> {
  try {
    ...
    // Manejo específico para cuando Docker no está disponible
    if (error instanceof Error && error.message.includes('Unable to locate executable file: docker')) {
...
export async function loginECR(registry: string, username: string, password: string): Promise<void> {
  try {
    ...
    for (const regData of regDatas) {
      ...
      const res = await Docker.getExecOutput(['login', '--password-stdin', '--username', regData.username, regData.registry], {
        ignoreReturnCode: true,
        silent: true,

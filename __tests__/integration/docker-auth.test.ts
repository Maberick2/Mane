...
describe('Docker Authentication Integration', () => {  
    ...
    test('should authenticate with Docker Hub', async () => {  
        ...
        await expect(docker.login(registry, username, password, 'false'))  
            .resolves  
            .not  
            .toThrow();  
    });  
    ...
    test('should fail with invalid credentials', async () => {  
        await expect(docker.login(registry, 'invalid_user', 'invalid_pass', 'false'))  
            .rejects  
            .toBeDefined();  
    });  
    ...
    test('should successfully logout', async () => {  
        ...
        const mockExecOutput = jest.spyOn(Docker, 'getExecOutput')  
    ...
    test('should handle Docker not available during logout', async () => {  
        ...
        jest.spyOn(Docker, 'getExecOutput')  
            .mockRejectedValueOnce(new Error('Unable to locate executable file: docker'));  
    ...
    test('should validate required credentials', async () => {  
        await expect(docker.loginStandard(registry, '', ''))  
            .rejects  
            .toThrow('Username and password required');  

        await expect(docker.loginStandard(registry, 'user', ''))  
            .rejects  
            .toThrow('Password required');  

        await expect(docker.loginStandard(registry, '', 'pass'))  
            .rejects  

...
const mockGetRegistriesData = aws.getRegistriesData as jest.MockedFunction<
  typeof aws.getRegistriesData
>;
...
describe('Docker Commands', () => {
  ...
  test('loginStandard validates credentials', async () => {
    await expect(loginStandard('registry.example.com', '', '')).rejects.toThrow(
      'Username and password required'
    );
    await expect(
      loginStandard('registry.example.com', 'user', '')
    ).rejects.toThrow('Password required');
    await expect(
      loginStandard('registry.example.com', '', 'pass')
  ...
  test('loginECR handles multiple registries', async () => {
    ...
    await loginECR(
      '012345678901.dkr.ecr.region-1.amazonaws.com',
      'AWS',
      'dummy-password'
    );

    expect(mockGetRegistriesData).toHaveBeenCalledWith(
      '012345678901.dkr.ecr.region-1.amazonaws.com'
    );
  ...
  test('handles Docker not available', async () => {
    jest
      ...
      .mockRejectedValue(new Error('Unable to locate executable file: docker'));

    await loginStandard('registry.example.com', 'user', 'pass');
    expect(core.warning).toHaveBeenCalledWith(
      'Docker not available - skipping login'
    );
  });
  ...
  test('logout handles Docker not available', async () => {
    ...
    await logout('registry.example.com');
    expect(core.warning).toHaveBeenCalledWith(
      'Docker not available - skipping logout'
    );
  });
